...
describe( "Y2K Earthquake V2" , function () {
    ...
    let basePrice = 100000000;
    let priceDecimal = 8;
    let priceTolerance = 15;
    let truthFrequencyTolerance = 24 * 60 * 60;
    let trackingFrequencyTolerance = 24 * 60 * 80;
    ...
    beforeEach(async function() {
        ...
        mockChainlinkOracle = await MockChainlinkOracle.deploy(basePrice, priceDecimal); 
        await mockChainlinkOracle.deployed();
        console.log("mochchainlink_oracle", mockChainlinkOracle.address)
        
        const ChainlinkTriggerFactory = await ethers.getContractFactory("ChainlinkTriggerFactory");
        chainlinkTriggerFactory = await ChainlinkTriggerFactory.deploy(ADMIN.address); 
        await chainlinkTriggerFactory.deployed();
        console.log("chainlinkTriggerFactory", chainlinkTriggerFactory.address)
        
        const ControllerPeggedAssetV2 = await ethers.getContractFactory("ControllerPeggedAssetV2");
        controller = await ControllerPeggedAssetV2.deploy(vaultFactoryV2.address, l2sequence, chainlinkTriggerFactory.address);
        ...
        const tiggerMetadata = {
            name : "Peg Protection Trigger",
            category : "Peg",
            description : "A trigger that protects from something depegging",
            logoURI : "https://via.placeholder.com/150"
        }

        const currentTimestamp = await time.latest()

        await mockChainlinkOracle.TEST_HOOK_setUpdatedAt(currentTimestamp);

        const trigger_tx = await chainlinkTriggerFactory["deployTrigger(int256,uint8,address,uint256,uint256,uint256,(string,string,string,string))"]
            (depegStrike, priceDecimal, mockChainlinkOracle.address, priceTolerance, trackingFrequencyTolerance, depegMarketId, tiggerMetadata);
        // const trigger_tx = await chainlinkTriggerFactory.deployTrigger(depegStrike, priceDecimal, mockChainlinkOracle.address, priceTolerance, trackingFrequencyTolerance, marketId, tiggerMetadata);
        const receipt_trigger = await trigger_tx.wait();

        const event_trigger = receipt_trigger.events.find((e) => e.event === "TriggerDeployed");
        deployedTrigger = event_trigger.args[0];

        console.log("deployedTrigger", deployedTrigger)
    ...
    it("test epoch with depeg event", async function(){
        ...
        await mockChainlinkOracle.TEST_HOOK_setUpdatedAt(currentTimestamp);
        await mockChainlinkOracle.TEST_HOOK_setPrice(150000000);

        await controller.triggerDepeg(depegMarketId, depegEpochId, {gasLimit : 3000000});

        const triggerState = await controller.getTriggerState(depegMarketId);
        expect(triggerState).to.be.equal(2);
