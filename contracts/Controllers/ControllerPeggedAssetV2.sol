...
contract ControllerPeggedAssetV2 {
    ...
    constructor(
        ...
        address _chainlinkTriggerFactory
    ) {
        ...
        chainlinkTriggerFactory = IChainlinkTriggerFactory(
            _chainlinkTriggerFactory
        );
    ...
    function triggerDepeg(uint256 _marketId, uint256 _epochId) public {
        ...
        if (state != MarketState.TRIGGERED) {
            revert MarketTriggered();
        }
        setTriggerState(_marketId);
        uint256 price = getTriggerPrice(_marketId);
    ...
    function setTriggerState(uint256 _marketId) public {
    ...
    function getTriggerState(
        uint256 _marketId
    ) public view returns (MarketState) {
        address trigger = chainlinkTriggerFactory.getTriggerByMarketId(
            _marketId
        );
    ...
    function getTriggerPrice(uint256 _marketId) public view returns (uint256) {
        ...
        return IChainlinkTrigger(trigger).getTrackingPriceAtTriggered();
    }
    ...
    function canExecDepeg(
        ...
        MarketState state = getTriggerState(_marketId);
        if (state != MarketState.TRIGGERED) return false;
    ...
    function getChainlinkTriggerFactory() external view returns (address) {
        return address(chainlinkTriggerFactory);
    }
