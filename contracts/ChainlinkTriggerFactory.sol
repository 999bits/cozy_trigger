...
contract ChainlinkTriggerFactory is IChainlinkTriggerFactory {
    ...
    bytes32 internal constant FIXED_PRICE_ORACLE_SALT =
        0xbeced09521047d05b8960b7e7bcc1d1292cf3e4b2a6b63f48335cbde5f7545d2;

    mapping(uint256 => address) public marketIdToTriggers;
    ...
    function deployTrigger(
        ...
        if (_truthOracle.decimals() != _trackingOracle.decimals())
    ...
    function deployTrigger(
        ...
        AggregatorV3Interface _truthOracle = deployFixedPriceAggregator(
            _price,
            _decimals
        );
    ...
    function deployFixedPriceAggregator(
        ...
        address _oracleAddress = computeFixedPriceAggregatorAddress(
            _price,
            _decimals
        );
        if (_oracleAddress.code.length > 0)
            return AggregatorV3Interface(_oracleAddress);
        return
            new FixedPriceAggregator{salt: FIXED_PRICE_ORACLE_SALT}(
                _decimals,
                _price
            );
    }
    ...
    function getTriggerByMarketId(
        ...
        return marketIdToTriggers[_marketId];
    }
}
